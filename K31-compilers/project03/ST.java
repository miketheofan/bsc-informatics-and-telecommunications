...
public class ST{
	...
	*/
	...
	/* The following function returns the offset of a method given. */
	public int getFunctionIndex(String className,String function){
		return this.classes.get(className).getFunctionIndex(function);
	}

	/* The following function returns the offset of a class given. */
	...
	public String getFieldTypeLLVM(String className,String field){

		classIdentifier tmpClass = this.classes.get(className);
		String tmp;

		if( (tmp = tmpClass.getFieldType(field)) == "" )
			return tmpClass.parentClass == "" ? "" : this.getFieldTypeLLVM(classes.get(tmpClass.parentClass).name,field);
		else
			return tmp;
